...
import { getAltPartRecommendation } from "../graphql/queries";
import { updateAltPartRecommendation } from "../graphql/mutations";
const client = generateClient();
export default function AltPartRecommendationUpdateForm(props) {
  const {
    id: idProp,
    altPartRecommendation: altPartRecommendationModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = altPartRecommendationRecord
      ? { ...initialValues, ...altPartRecommendationRecord }
      : initialValues;
    setMPN(cleanValues.MPN);
    setDescription(cleanValues.Description);
    setAltMPN1(cleanValues.AltMPN1);
    setAltMPN2(cleanValues.AltMPN2);
    setAltMPN3(cleanValues.AltMPN3);
    setLifecycleStatus(cleanValues.LifecycleStatus);
    setROHS(cleanValues.ROHS);
    setEOL(cleanValues.EOL);
    setErrors({});
  };
  const [altPartRecommendationRecord, setAltPartRecommendationRecord] =
    React.useState(altPartRecommendationModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getAltPartRecommendation.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getAltPartRecommendation
        : altPartRecommendationModelProp;
      setAltPartRecommendationRecord(record);
    };
    queryData();
  }, [idProp, altPartRecommendationModelProp]);
  React.useEffect(resetStateValues, [altPartRecommendationRecord]);
  const validations = {
    MPN: [{ type: "Required" }],
    Description: [],
    AltMPN1: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          AltMPN1: AltMPN1 ?? null,
          AltMPN2: AltMPN2 ?? null,
          AltMPN3: AltMPN3 ?? null,
        ...
        try {
          ...
          await client.graphql({
            query: updateAltPartRecommendation.replaceAll("__typename", ""),
            variables: {
              input: {
                id: altPartRecommendationRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "AltPartRecommendationUpdateForm")}
      {...rest}
    >
      <TextField
        label="Mpn"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || altPartRecommendationModelProp)}
