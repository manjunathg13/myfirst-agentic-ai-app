...
import { listAltPartRecommendations } from "../graphql/queries";
...
export default function LifeCycleStatusCollection(props) {
    ...
    while ((newCache.length < cacheUntil || !isPaginated) && newNext != null) {
      ...
      const result = (
        await client.graphql({
          query: listAltPartRecommendations.replaceAll("__typename", ""),
          variables,
        })
      ).data.listAltPartRecommendations;
  ...
  return (
    <div>
      <Collection
        ...
        {...getOverrideProps(overrides, "LifeCycleStatusCollection")}
        {...rest}
      >
        {(item, index) => {
          if (loading) {
